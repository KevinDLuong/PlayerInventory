...
public class Player{
	...
	private void addCustomItem(){
		//inventory[1]= ... TODO FILL IN YOUR Custom ITEM
	...
	protected void executeCommand(String command, RoomObject obj, Item itm){
		switch(command){
			...
			case "SIT":
				sit(obj,itm); break;
			case "THROW":
				throwItem(obj,itm); break;
			case "HELP":
				System.out.println("You can attack, sit, eat, look, inv, throw");
				break;
	...
	private void throwItem(RoomObject ro, Item itm){
		if(ro==null){
			System.out.println("You start throwing a fit");
		} else if(ro instanceof ThrowableItem){
			((ThrowableItem)ro).throwItem(itm);
		} else {
			System.out.println("You're too weak to throw the "+ro);
		}
	}
	//TODO: Complete the custom method
	//private void myCustomMethod(RoomObject ro, Item itm){
	//	...
	//}
